...
if (!$aluno_id) {
    echo json_encode(['erro' => 'Aluno não identificado.']);
    exit;
}
...
if ($acao === 'confirmar' && isset($input['letra']) && isset($input['pergunta_id'])) {
    ...
    $stmt = $conn->prepare('INSERT INTO auditoria (acao, aluno_nome, data_hora, instituicao_nome, pergunta_id, resposta_dada, resposta_correta, aluno_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    $stmt->bind_param('ssssissi', $acao_auditoria, $aluno_nome, $data_hora, $instituicao_nome, $pergunta['id'], $resposta_dada, $resposta_correta, $aluno_id);
    $stmt->execute();
    $stmt->close();
